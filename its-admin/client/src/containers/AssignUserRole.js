...
class AssignUserRole extends Component {
    ...
    applyForRoleWithLevel(level = 0){
        this.props.applyForRoleWithLevel(this.props.currentUser.uid, level);
    }
...
const mapDispatchToProps = (dispatch) => {
    return {
        applyForRoleWithLevel: function(userID, level = 0){
            let selectedRoleAndLevel = {
                role: level > 0 ? "tech" : "helpdesk",
                role_level: level
            }
            dispatch(StaffActions.updateStaffProfile(userID, selectedRoleAndLevel));
        }
    }
}
